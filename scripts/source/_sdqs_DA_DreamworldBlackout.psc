...
{Condition that must be satisfied for the quest to fire. Should be overloaded in the childs}
	...
	if (Utility.RandomInt(0,100)<=_SDGVP_health_threshold.GetValue()) && ( _SDGVP_Sanguine_Blessing.GetValue() >0) && ( (akAggressor.HasKeyword( _SDKP_actorTypeNPC ) || (akAggressor.GetRace() == falmerRace)) && funct.checkGenderRestriction( akAggressor, akPlayer ) ) && !funct.actorFactionInList( akAggressor, _SDFLP_banned_factions ) 
		return IsStopped()
	else
		return false
	endif

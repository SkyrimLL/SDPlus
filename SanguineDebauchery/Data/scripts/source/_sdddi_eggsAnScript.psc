Scriptname _sdddi_eggsAnScript extends zadPlugScript  
...
Function OnEquippedPre(actor akActor, bool silent=false)
	...
	if akActor == libs.PlayerRef
		if Aroused.GetActorExposure(akActor) < libs.ArousalThreshold("Desire")
			msg = "The eggs fits snugly inside your hole, spreading a wave of pleasure deep into your guts."
		elseif Aroused.GetActorExposure(akActor) < libs.ArousalThreshold("Horny")
			msg = "You carefully push the eggs through your anal ring, your lust growing as they stretc you and slide in."
		elseif Aroused.GetActorExposure(akActor) < libs.ArousalThreshold("Desperate")
			msg = "You eagerly spread your ass cheeks to stretch yourself further as the egg push way deep into your quivering hole, making you maon excitedly."
		else
	...
	elseif msgChoice==2 ; Not wearing a belt, plugs
		...
		if ( Aroused.GetActorArousal(libs.PlayerRef) <= 10 ) ; libs.ArousalThreshold("Desire")
			...
			elseif ( Aroused.GetActorArousal(libs.PlayerRef) < 80 ) ; libs.ArousalThreshold("Desperate")
				msg = "As you try scooping the slimy eggs through your tight opening, you can feel stretch and occupy more of your anal cavity, making them impossible to remove."
				libs.UpdateExposure(libs.PlayerRef,2)
			else
				msg = "You desperately try to pull the glistening eggs out of your abused hole, only to feel your own ass clenching painfully around it and keeping them deeply buried inside you."
				libs.UpdateExposure(libs.PlayerRef,2)
			endif
			libs.NotifyPlayer(msg, true)
...
Function NoKeyFailMessage(Actor akActor)
	if akActor == libs.PlayerRef
		libs.NotifyPlayer("Try as you might, the belt you are wearing prevents you from removing the eggs.", true)
	Else
		libs.NotifyNPC("The belt that "+akActor.GetLeveledActorBase().GetName() + " is wearing is securely locking the eggs in place. You must remove it prior to removing them.", true)
	EndIf
EndFunction

Scriptname _SDDA_BlackoutEnslavementCreature extends daymoyl_QuestTemplate  
...
{Condition that must be satisfied for the quest to fire. Should be overloaded in the childs}
	Debug.Trace("[SDDA] Blackout Enslavement Creature: condition")

	thisLocation = akLocation
	thisPlayer = Game.GetPlayer()
	thisAggressor = akAggressor

	Debug.Trace("[SDDA] Blackout Enslavement Creature start enslavement attempt (Humanoid):" + thisAggressor)
	Debug.Trace("	start master is Creature:" + StorageUtil.GetIntValue( thisAggressor, "_SD_bIsSlaverCreature"))
	
	if (Utility.RandomInt(1,100)<=StorageUtil.GetIntValue(thisPlayer, "_SD_iChanceEnslavement")) && (StorageUtil.GetIntValue(thisPlayer, "_SD_iEnslavementInitSequenceOn")!=1) && (StorageUtil.GetIntValue( thisAggressor, "_SD_bIsSlaverCreature") == 1)  ; Simplified check for DA only - SD mod event will handle complex faction checks
		return true
	else
		return false
	endif
...
bool Function QuestStart(Location akLocation, Actor akAggressor, Actor akFollower)
	Debug.Trace("[SDDA] Blackout Enslavement Creature: selected")

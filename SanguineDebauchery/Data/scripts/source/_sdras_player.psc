...
State monitor
	...
	Event OnPlayerLoadGame()
		...
		kPlayer = Self.GetReference() as Actor
		current_crc = FNIS_aa.GetInstallationCRC()

		; Force refresh on game load in case of changes
	  	; if ( current_crc == 0 )    ; Installation Error: no AA generated by FNIS  
	  	; elseif ( current_crc != fctConstraints.ModCRC )
	      	fctConstraints.ModID = FNIS_aa.GetAAModID("sdc", "sanguineDebauchery",true) ;true:testonly    
	      	fctConstraints.mtidle_base=FNIS_aa.GetGroupBaseValue(fctConstraints.ModID,FNIS_aa._mtidle(),"sanguineDebauchery", true) 
			...
			fctConstraints.sneakmt_base=FNIS_aa.GetGroupBaseValue(fctConstraints.ModID,FNIS_aa._sneakmt(),"sanguinesDebauchery",true) 
	      	...
	      	fctConstraints.ABC_mtidle = FNIS_aa.GetGroupBaseValue(fctConstraints.ABC_ModID, FNIS_aa._mtidle(), "DeviousDevices",true)

			; Debug.Trace("[SD] Crawl override loaded")
			; Debug.Notification("[SD] Crawl override loaded")

...
;BEGIN CODE
; 2017-04-26 - _SD_BountySlave quest disabled for now. Revisit later for compatibility with XPO
...
Function questShutdown()
	...
	While idx < _SDRAP_companions.Length
		...
		If ( nthActor )
			...
			fctOutfit.clearDeviceNPCByString ( nthActor, sDeviceString = "Armbinder", sOutfitString = "" )
		EndIf
		idx += 1
	EndWhile
...
Function removeSlaveItems(  Bool bCollar = True,  Bool bBindings = True, Bool bPunish = True,Bool bFollowers = True  )
	Actor master = Alias__SDRA_master.GetReference() as Actor
	Actor slave = Alias__SDRA_slave.GetReference() as Actor
	...
	If (bBindings )
		...
		fctOutfit.clearDeviceByString ( "Gloves" , "" )
		Utility.Wait(2.0)
	...
	If (bPunish )
		...
		fctOutfit.ClearSlavePunishment(slave , "Belt" , true )
		fctOutfit.ClearSlavePunishment(slave , "Blindfold" , true )
		fctOutfit.ClearSlavePunishment(slave , "Gag" , true )
		fctOutfit.ClearSlavePunishment(slave , "GagPanel" , true )
		fctOutfit.ClearSlavePunishment(slave , "PlugAnal" , true )
		fctOutfit.ClearSlavePunishment(slave , "PlugVaginal" , true )
		fctOutfit.ClearSlavePunishment(slave , "WristRestraints" , true )
		...
		While idx < _SDRAP_companions.Length
			...
			If ( nthActor )
				...
				If (bCollar)
					...
					fctOutfit.clearDeviceNPCByString ( nthActor, "Collar", sOutfitString = "" )
				Endif
				If (bBindings )
					; nthActor.SendModEvent("SDClearDevice","Armbinder:zap")
					fctOutfit.clearDeviceNPCByString ( nthActor, "Armbinder", sOutfitString = "" )
				Endif
				If (bPunish )
					; nthActor.SendModEvent("SDClearDevice","Gag:zap")
					fctOutfit.clearDeviceNPCByString ( nthActor, "Gag", sOutfitString = "" )
				Endif

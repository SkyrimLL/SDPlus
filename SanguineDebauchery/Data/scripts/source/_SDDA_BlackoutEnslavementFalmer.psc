...
{Condition that must be satisfied for the quest to fire. Should be overloaded in the childs}
	...
	if (Utility.RandomInt(1,100)<=StorageUtil.GetIntValue(thisPlayer, "_SD_iChanceEnslavement")) && (StorageUtil.GetIntValue(thisPlayer, "_SD_iEnslavementInitSequenceOn")!=1) && (StorageUtil.GetIntValue( thisAggressor, "_SD_bIsSlaverFalmer") == 1)  ; Simplified check for DA only - SD mod event will handle complex faction checks
		return true
	else
		return false
	endif

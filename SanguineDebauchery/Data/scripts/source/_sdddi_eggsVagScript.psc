Scriptname _sdddi_eggsVagScript extends zadPlugScript  
...
Function NoKeyFailMessage(Actor akActor)
	if akActor == libs.PlayerRef
		libs.NotifyPlayer("Try as you might, the belt you are wearing prevents you from removing the worm.", true)
	Else
		libs.NotifyNPC("The belt that "+akActor.GetLeveledActorBase().GetName() + " is wearing is securely locking the worm in place. You must remove it prior to removing them.", true)
	EndIf
EndFunction
